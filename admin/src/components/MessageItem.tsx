    ...
    <div className="message-attachments" style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '8px' }}>
      {message.attachments.map((att, index) => (
        <div key={index} className="attachment-item">
          {att.type === 'image' ? (
            <a href={att.url || (att as any).fileURL} target="_blank" rel="noreferrer" className="message-image-link">
              <img src={getDisplayURL(att.url || (att as any).fileURL)} alt={att.name} className="message-image" style={{ maxWidth: '100%', borderRadius: '8px' }} />
            </a>
          ) : (
            <div className="message-file" style={{ background: 'rgba(0,0,0,0.03)', borderRadius: '8px', padding: '8px' }}>
              <div className="file-icon">FILE</div>
              <div className="file-details">
                <a href={att.url || (att as any).fileURL} target="_blank" rel="noreferrer" className="file-name">
                  {att.name}
                </a>
